...
import { Actor, IResourceRepository, Issue } from '@gittrends/lib';
...
export class IssuesRepository implements IResourceRepository<Issue> {
  ...
  async findByRepository(repository: string, opts?: { limit: number; skip: number }): Promise<Issue[]> {
  ...
  async save(issue: Issue | Issue[], trx?: Knex.Transaction): Promise<void> {
    const data = (Array.isArray(issue) ? issue : [issue]).map((issue) => {
      ...
      if (issue.assignees) issue.assignees = issue.assignees.map((a) => (a instanceof Actor ? a.id : a));
      if (issue.author instanceof Actor) issue.author = issue.author.id;
      if (issue.editor instanceof Actor) issue.editor = issue.editor.id;
      if (issue.participants) issue.participants = issue.participants.map((a) => (a instanceof Actor ? a.id : a));
