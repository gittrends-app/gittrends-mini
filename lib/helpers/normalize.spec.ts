...
describe('Normalize response received from GitHub API', () => {
  test('it should transform date strings to objects', () => {
    ...
    expect(
      isEqual({ tested_on: date }, normalize({ tested_on: date.toISOString() }))
    ).toBe(true);
    expect(
      isEqual({ date: date }, normalize({ date: date.toISOString() }))
    ).toBe(true);
    expect(
      isEqual([{ date: date }], normalize([{ date: date.toISOString() }]))
    ).toBe(true);
  ...
  test('it should spread single properties', () => {
    ...
    expect(normalize({ a: { id: 1, name: 2 } })).toStrictEqual({
      a: { id: 1, name: 2 },
    });
